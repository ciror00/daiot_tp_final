...
class MqttHandler:
    def __init__(self, server, user='', passw='', topic_sub='', topic_pub=''):
    ...
    def connect_and_subscribe(self):
        client = MQTTClient(client_id = self._client_id, server=self._mqtt_server, port=0, user=self._user, password=self._password)
        ...
        print(message.format(self._mqtt_server, self._topic_sub))
    ...
    def publish(self, topic_pub, msg=None):
        self.client.publish(self._topic_pub, msg)
